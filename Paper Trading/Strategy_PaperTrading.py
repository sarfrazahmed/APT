...
###############################################################
def long_entry(data, index, sl, tp):
...
## Function to Execute Long Entry
###############################################################
def short_entry(data, index, sl, tp):
    data.Order_Status[index] = 'Entry'
...
def GapUpStrategy(data, order_status, order_signal,
                  ...
                  stop_loss, target, lot_size, target_profit_1, skip_date):
    ...
    #     prev_day_close = copy.deepcopy(ads_iteration.Close[i])
            ...
            if order_status == 'Exit':
                ...
                if data.Close[0] > entry_high_target:
                    ...
                    order_signal = data.Order_Signal[i]
                    target = data.Target[i]
                    stop_loss = data.Stop_Loss[i]
                    order_price = data.Order_Price[i]
